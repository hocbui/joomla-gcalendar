...
class GCalendarConfig extends DefaultCalendarConfig{

	var $showSelectionList = true;

	function GCalendarConfig($model){
		$this->DefaultCalendarConfig($model);
	}

	function printToolBar(){
		$document =& JFactory::getDocument();
		$document->setTitle('GCalendar: '.$this->getViewTitle());
		...
		switch($view) {
			...
			$calCode .= "var d = jQuery.datepicker.parseDate('".$this->getDateFormat()."', dateField.value);\n";
			...
			$calCode .= "document.getElementById('gcdate').value = jQuery.datepicker.formatDate('".$this->getDateFormat()."', new Date(".$year.", ".$month." - 1, ".$day."));\n";
			$calCode .= "jQuery(\"#gcdate\").datepicker({dateFormat: '".$this->getDateFormat()."'});\n";
	...
	function printCalendarSelectionList($feeds){
		...
		$this->image("btn-down.png", "expand", "gc_gcalendar_view_toggle_status");
		echo "</a></div>\n";
	}

...
C=o(G.visStart);u.find("td").each(function(){var W=m(this);if(Z>1)C.getMonth()==v?W.removeClass("fc-other-month"):W.addClass("fc-other-month");+C==+x?W.removeClass("fc-not-today").addClass("fc-today").addClass(R+"-state-highlight"):W.addClass("fc-not-today").removeClass("fc-today").removeClass(R+"-state-highlight");W.find("div.fc-day-number").text(C.getDate());D(C,1);ga&&ba(C)});if(Z==1){C=o(G.visStart);na.find("th").each(function(){m(this).text(oa(C,K,b));this.className=this.className.replace(/^fc-\w+(?= )/,
...
sa.empty()}function s(n){var k=o(G.visStart),K=D(o(k),Q),g=m.map(n,Pa),v,x,q,C,M,W,T=[];for(v=0;v<Z;v++){x=Za(G.sliceSegs(n,g,k,K));for(q=0;q<x.length;q++){C=x[q];for(M=0;M<C.length;M++){W=C[M];W.row=v;W.level=q;T.push(W)}}D(k,7);D(K,7)}return T}function I(n,k){mb(n,Z,G,0,ca,function(K){return u.find("tr:eq("+K+")")},U.left,U.right,sa,B,k)}function B(n,k,K){G.eventElementHandlers(n,k);if(n.editable||n.editable===J&&b.editable){S(n,k);K.isEnd&&G.resizableDayEvent(n,k,L)}}function S(n,k){if(!b.disableDragging&&
...
E(G.find("td"));Ma=m("<div style='position:absolute;z-index:8;top:0;left:0'/>").appendTo(n);i=o(d);H="<div class='fc-agenda-bg' style='position:absolute;z-index:1'><table style='width:100%;height:100%'><tr class='fc-first'>";for(p=0;p<g;p++){H+="<td class='fc-"+za[i.getDay()]+" "+ma+"-state-default "+(!p?"fc-leftmost ":"")+(+i==+t?ma+"-state-highlight fc-today":"fc-not-today")+"'><div class='fc-day-content'><div>&nbsp;</div></div></td>";D(i,X);La&&ba(i,X)}H+="</tr></table></div>";K=m(H).appendTo(a)}}
function h(){var d=xb(),f=o(d);f.setHours(b.firstHour);var i=L(d,f)+1;d=function(){G.scrollTop(i)};d();setTimeout(d,0)}function l(d,f){W=d;eb={};G.height(d-V.height());C=G.find("tr:first div").height()+1;K.css({top:V.find("tr").height(),height:d});f&&h()}function j(d){M=d;Qa.clear();G.width(d);k.width("");d=V.find("tr:first th");var f=K.find("td"),i=G[0].clientWidth;k.width(i);x=0;Ja(V.find("tr:lt(2) th:first").add(G.find("tr:first th")).width("").each(function(){x=Math.max(x,m(this).outerWidth())}),
x);q=Math.floor((i-x)/g);Ja(f.slice(0,-1),q);Ja(d.slice(1,-2),q);Ja(d.slice(-2,-1),i-x-q*(g-1));K.css({left:x,width:i-x})}function F(d){d.click(s).mousedown(ga)}function E(d){d.click(s).mousedown(ea)}function s(d){if(!w.option("selectable")){var f=Math.floor((d.pageX-K.offset().left)/q);f=D(o(w.visStart),qa+X*f);var i=this.className.match(/fc-slot(\d+)/);if(i){i=parseInt(i[1])*b.slotMinutes;var t=Math.floor(i/60);f.setHours(t);f.setMinutes(i%60+Ba);w.trigger("dayClick",this,f,false,d)}else w.trigger("dayClick",
...
$a(function(f){ta.mouse(f)},function(f){Fa.dragStop(f)});Fa.dragStart(d);ta.mouse(d);return false}}function ea(d){if(w.option("selectable")){ta=u(function(f){if(f){f=ia(f.row,f.origCol);Ga.drag(f,da(o(f),b.slotMinutes),false)}else Ga.drag()});$a(function(f){ta.mouse(f)},function(f){Ga.dragStop(f)});Ga.dragStart(d);ta.mouse(d);return false}}function fa(){Ga.unselect();Fa.unselect()}function ka(d,f){var i=w.option("selectHelper");if(i){var t=Aa(d,w.visStart);if(t>=0&&t<g){t=ta.rect(0,t*X+qa,1,t*X+qa+
...
ra=null}}function ua(d,f,i){var t;if(Sa){t=Aa(i,w.visStart)*X+qa+1;f=Aa(f,w.visStart)*X+qa+1}else{t=Aa(f,w.visStart);f=Aa(i,w.visStart)}t=Math.max(0,t);f=Math.min(g,f);if(t<f){d=d.rect(0,t,1,f,V);F(w.renderOverlay(d,V))}}function Z(d,f,i){for(var t=o(w.visStart),p=D(o(t),1),z=0;z<g;z++){var y=new Date(Math.max(t,f)),H=new Date(Math.min(p,i));if(y<H){var N=d.rect(0,z*X+qa,1,z*X+qa+1,n);y=L(t,y);H=L(t,H);N.top=y;N.height=H-y;E(w.renderOverlay(N,n))}D(t,1);D(p,1)}}function Q(){w.clearOverlays()}function L(d,
...
Math.max(vb,La)+g)%g*X+qa}function U(d){return D(o(w.visStart),d*X+qa)}function ia(d,f){f=U(f);da(f,Ba+d*b.slotMinutes);return f}var V,G,n,k,K,g,v=0,x,q,C,M,W,T,xa=[],db,Ma,ma,vb,La,Sa,X,qa,Ba,Ta,Qa=new nb(function(d){return K.find("td:eq("+d+") div div")}),eb={},Fa,Ga,ra,ta,w=m.extend(this,ob,e,{renderAgenda:c,renderEvents:I,rerenderEvents:B,clearEvents:S,setHeight:l,setWidth:j,beforeHide:function(){T=G.scrollTop()},afterShow:function(){G.scrollTop(T)},defaultEventEnd:function(d){var f=o(d.start);
...
x=function(){--v||g&&g(U)},q=0;q<c.length;q++)A(c[q],x)}function A(g,v){var x=u.name,q=o(L),C=function(T){if(x==u.name&&+q==+L&&m.inArray(g,c)!=-1){for(var xa=0;xa<T.length;xa++){Ha(T[xa],a);T[xa].source=g}U=U.concat(T);v&&v(T)}},M=function(T){C(T);pa()};if(typeof g=="string"){var W={};W[a.startParam]=Math.round(ia.getTime()/1E3);W[a.endParam]=Math.round(V.getTime()/1E3);W.browserTimezone=V.getTimezoneOffset();if(a.cacheParam)W[a.cacheParam]=(new Date).getTime();ha();m.ajax({url:g,dataType:"json",
data:W,cache:a.cacheParam||false,success:M})}else if(m.isFunction(g)){ha();g(o(ia),o(V),M)}else g?C(g):alert("Error detected in fullcalender fetchEventSource()\nPlease check the last comma at the end of your feed list.")}function P(){$(function(g){u.renderEvents(g)})}function ha(){G++||u.trigger("loading",ea,true)}function pa(){--G||u.trigger("loading",ea,false)}function la(g){if(g){var v=m("<tr/>");m.each(g.split(" "),function(x){x>0&&v.append("<td><span class='fc-header-space'/></td>");var q;m.each(this.split(","),
function(C,M){if(M=="title"){v.append("<td><h2 class='fc-header-title'>&nbsp;</h2></td>");q&&q.addClass(h+"-corner-right");q=null}else{var W;if(n[M])W=n[M];else if(Oa[M])W=function(){T.removeClass(h+"-state-hover");l(M)};if(W){q&&q.addClass(h+"-no-right");var T;C=a.theme?kb(a.buttonIcons,M):null;var xa=kb(a.buttonText,M);if(C)T=m("<div class='fc-button-"+M+" ui-state-default'><a><span class='ui-icon ui-icon-"+C+"'/></a></div>");else if(xa)T=m("<div class='fc-button-"+M+" "+h+"-state-default'><a><span>"+
xa+"</span></a></div>");if(T){T.click(function(){T.hasClass(h+"-state-disabled")||W()}).mousedown(function(){T.not("."+h+"-state-active").not("."+h+"-state-disabled").addClass(h+"-state-down")}).mouseup(function(){T.removeClass(h+"-state-down")}).hover(function(){T.not("."+h+"-state-active").not("."+h+"-state-disabled").addClass(h+"-state-hover")},function(){T.removeClass(h+"-state-hover").removeClass(h+"-state-down")}).appendTo(m("<td/>").appendTo(v));q?q.addClass(h+"-no-right"):T.addClass(h+"-corner-left");
q=T}}}});q&&q.addClass(h+"-corner-right")});return m("<table/>").append(v)}}function wa(){ua=a.contentHeight?a.contentHeight:a.height?a.height-(k?k.height():0)-fb(ca[0]):Math.round(ca.width()/Math.max(a.aspectRatio,0.5))}function R(g){Q++;u.setHeight(ua,g);if(sa){sa.css("position","relative");sa=null}u.setWidth(ca.width(),g);Q--}function ja(){if(!Q)if(u.start){var g=++Z;setTimeout(function(){if(g==Z&&!Q&&F())if(ka!=(ka=fa.outerWidth())){Q++;S();u.trigger("windowResize",ea);Q--}},200)}else ga()}function ga(){setTimeout(function(){!u.start&&
E()&&j()},0)}var ea=this,fa=m(ea).addClass("fc"),ka,ca=m("<div class='fc-content "+h+"-widget-content' style='position:relative'/>").prependTo(ea),ua,Z=0,Q=0,L=new Date,na,u,va={},sa;a.isRTL&&fa.addClass("fc-rtl");a.theme&&fa.addClass("ui-widget");if(a.year!==J&&a.year!=L.getFullYear()){L.setDate(1);L.setMonth(0);L.setFullYear(a.year)}if(a.month!==J&&a.month!=L.getMonth()){L.setDate(1);L.setMonth(a.month)}a.date!==J&&L.setDate(a.date);var U=[],ia,V,G=0,n={render:function(){wa();r();B();j()},changeView:l,
getView:function(){return u},getDate:function(){return L},option:function(g,v){if(v===J)return a[g];if(g=="height"||g=="contentHeight"||g=="aspectRatio"){a[g]=v;S()}},destroy:function(){m(window).unbind("resize",ja);k&&k.remove();ca.remove();m.removeData(ea,"fullCalendar")},prev:function(){j(-1)},next:function(){j(1)},prevYear:function(){gb(L,-1);j()},nextYear:function(){gb(L,1);j()},today:function(){L=new Date;j()},gotoDate:function(g,v,x){if(typeof g=="object")L=o(g);else{g!==J&&L.setFullYear(g);
v!==J&&L.setMonth(v);x!==J&&L.setDate(x)}j()},incrementDate:function(g,v,x){g!==J&&gb(L,g);v!==J&&hb(L,v);x!==J&&D(L,x);j()},updateEvent:function(g){var v,x=U.length,q,C=g.start-g._start,M=g.end?g.end-(g._end||u.defaultEventEnd(g)):0;for(v=0;v<x;v++){q=U[v];if(q._id==g._id&&q!=g){q.start=new Date(+q.start+C);q.end=g.end?q.end?new Date(+q.end+M):new Date(+u.defaultEventEnd(q)+M):null;q.title=g.title;q.url=g.url;q.allDay=g.allDay;q.className=g.className;q.editable=g.editable;Ha(q,a)}}Ha(g,a);I()},renderEvent:function(g,
v){Ha(g,a);if(!g.source){if(v)(g.source=c[0]).push(g);U.push(g)}I()},removeEvents:function(g){if(g){if(!m.isFunction(g)){var v=g+"";g=function(q){return q._id==v}}U=m.grep(U,g,true);for(x=0;x<c.length;x++)if(typeof c[x]=="object")c[x]=m.grep(c[x],g,true)}else{U=[];for(var x=0;x<c.length;x++)if(typeof c[x]=="object")c[x]=[]}I()},clientEvents:function(g){if(m.isFunction(g))return m.grep(U,g);else if(g){g+="";return m.grep(U,function(v){return v._id==g})}return U},rerenderEvents:I,addEventSource:function(g){c.push(g);
A(g,I)},removeEventSource:function(g){c=m.grep(c,function(v){return g&&v!=g});U=m.grep(U,function(v){return g&&v.source!=g});I()},refetchEvents:function(){$(I)},select:function(g,v,x){u.select(g,v,x===J?true:x)},unselect:function(){u.unselect()}};m.data(this,"fullCalendar",n);var k,K=a.header;if(K)k=m("<table class='fc-header'/>").append(m("<tr/>").append(m("<td class='fc-header-left'/>").append(la(K.left))).append(m("<td class='fc-header-center'/>").append(la(K.center))).append(m("<td class='fc-header-right'/>").append(la(K.right)))).prependTo(fa);
m(window).resize(ja);l(a.defaultView);E()||ga()});return this};var Db=0;Cb({weekMode:"fixed"});Oa.month=function(a,b){return new Xa(a,b,{render:function(e,c){if(c){hb(e,c);e.setDate(1)}e=this.start=o(e,true);e.setDate(1);this.end=hb(o(e),1);var h=this.visStart=o(e);c=this.visEnd=o(this.end);var l=b.weekends?0:1;if(l){ba(h);ba(c,-1,true)}D(h,-((h.getDay()-Math.max(b.firstDay,l)+7)%7));D(c,(7-c.getDay()+Math.max(b.firstDay,l))%7);h=Math.round((c-h)/(Bb*7));if(b.weekMode=="fixed"){D(c,(6-h)*7);h=6}this.title=
oa(e,this.option("titleFormat"),b);this.renderGrid(h,b.weekends?7:5,this.option("columnFormat"),true)}})};Oa.basicWeek=function(a,b){return new Xa(a,b,{render:function(e,c){c&&D(e,c*7);e=this.visStart=o(this.start=D(o(e),-((e.getDay()-b.firstDay+7)%7)));c=this.visEnd=o(this.end=D(o(e),7));if(!b.weekends){ba(e);ba(c,-1,true)}this.title=Ea(e,D(o(c),-1),this.option("titleFormat"),b);this.renderGrid(1,b.weekends?7:5,this.option("columnFormat"),false)}})};Oa.basicDay=function(a,b){return new Xa(a,b,{render:function(e,
...
12?"AM":"PM"},u:function(a){return oa(a,"yyyy-MM-dd'T'HH:mm:ss'Z'")},S:function(a){a=a.getDate();if(a>10&&a<20)return"th";return["st","nd","rd"][a%10-1]||"th"}},Va,J,za=["sun","mon","tue","wed","thu","fri","sat"]})(jQuery);
